...
int main()
{
  ...
  if(ans_euler.empty()) return -1;  // ans=IMPOSSIBLE, de a megoldás utat ad meg
  if(sol_euler.size() != m+1) return -1;
  if(sol_euler.front() != 1 || sol_euler.back() != 1) return -1;  // körsétának az 1-es csúcsból kell indulnia
